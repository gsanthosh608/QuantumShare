        ...
        try {
            ...
            const responses = await Promise.all(platforms.map(async platform => {
                ...
                try {
                    ...
                    if (platform === 'facebook' && Array.isArray(response.data)) {
                        response.data.forEach(async res => {
                            if (res.status === "success" && res.platform === "facebook") {
                                ...
                                dispatch(setRemainingCredits(remainingCredits));
                                sessionStorage.setItem('remainingCredits', res.remainingCredits);
                        ...
                        dispatch(setRemainingCredits(remainingCredits));
                        sessionStorage.setItem('remainingCredits', res.remainingCredits)
                    } else if (platform === 'LinkedIn' && response.data.structure?.status === "error" && response.data.structure.code === 114) {
                        const res = response.data.structure;
                        console.error('Credit Depleted Error Message:', res.message);
                        toast.error(res.message);
                    } 
                    ...
                    if (error.response?.status === 403) {
                    ...
                    } else if (platform === 'instagram' && responseData.code === 116) {
                        ...
                        toast.error(responseData.structure.message);
                    } else if (platform === 'telegram' && responseData.structure?.status === "error" && responseData.structure.code === 114) {
                        const err = responseData.structure;
                        console.error('Credit Depleted Error Message:', err.message);
                        toast.error(err.message);
                    } else if (platform === 'LinkedIn' && responseData.structure?.code === 404) {
                        toast.error(responseData.structure.message);
                    } else if (platform === 'LinkedIn' && responseData.structure?.status === "error" && responseData.structure.code === 114) {
                        const err = responseData.structure;
                        console.error('Credit Depleted Error Message:', err.message);
                        toast.error(err.message);
